...
export async function getHistoryForMonth(source, yearMonth) {
  const db = await getDb();
  if (!db) return [];
  await ensureSchema();
  ...
  const result = await db.execute({
    sql: `
      ...
      ORDER BY date ASC
    `,
    args: [source, `${yearMonth}%`],
  });

  return mapRows(result.rows);
}

export async function getHistoryForYear(source, year) {
  const db = await getDb();
  if (!db) return [];
  await ensureSchema();
...
export async function getAvailableYears(source) {
  const db = await getDb();
  if (!db) return [];
  await ensureSchema();
  ...
  return result.rows.map((row) => String(row.year));
}
